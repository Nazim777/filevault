        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/30">
              {["User", "Plan", "Files", "Folders", "Verified", "Joined"].map((h) => (
            ...
            {filtered.map((user) => {
              ...
              return (
                <tr key={user.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    {sub ? (
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium border ${TIER_COLORS[sub.package.tier]}`}>
                        {sub.package.name}
                      </span>
                    ) : (
                  ...
                  <td className="px-5 py-3.5 font-medium">{user._count.files}</td>
                  <td className="px-5 py-3.5 font-medium">{user._count.folders}</td>
                  <td className="px-5 py-3.5">
                    <Badge variant={user.isEmailVerified ? "success" : "warning"}>
                      {user.isEmailVerified ? "Yes" : "No"}
                    </Badge>
                  </td>
                  <td className="px-5 py-3.5 text-muted-foreground">{formatDate(user.createdAt, true)}</td>
                </tr>
              );
            })}
          </tbody>
